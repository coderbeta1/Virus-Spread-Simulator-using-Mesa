...
def compute_immune_count(model):
    ...
    for agent in model.schedule.agents:
        if (agent.infection_status == "immune"):
            susceptible_count+=1
    return susceptible_count
def compute_dead_count(model):
    susceptible_count = 0
    for agent in model.schedule.agents:
        if (agent.infection_status == "dead"):
            susceptible_count+=1
    return susceptible_count
def compute_economy(model):
    total_economy = 0
    for agent in model.schedule.agents:
        total_economy+=agent.wealth
    return int(total_economy)

def compute_severe_count(model):
    susceptible_count = 0
    for agent in model.schedule.agents:
        if (agent.infection_severity == "severe"):
            susceptible_count+=1
    return susceptible_count
def compute_hospitalization_count(model):
    susceptible_count = 0
    for agent in model.schedule.agents:
        if (agent.infection_severity == "hospitalization"):
            susceptible_count+=1
    return susceptible_count
...
class VirusSpreadModel(Model):
    ...
    minimum_income, minimum_expense, amplitudes):
        ...
        infection_probability_randomised = numpy.random.normal(self.prob_spread, 0.1, population_size)
...
class PersonAgent(Agent):
    ...
    def move(self):
        if self.infection_status == "dead" or (self.infection_status == "infected"
        ...
        if self.infection_status == "infected":
            ...
            if self.infection_severity == "asymptomatic":
                ...
                if economyModel.age_severe_probs[indice] > teste_sub:
                    ...
                    # self.get_statistics()
                    # print(compute_hospitalization_count(self.models)+compute_severe_count(self.models))
                    if compute_severe_count(self.models) + compute_hospitalization_count(self.models) >= self.model.hospital_limit:
                        self.infection_status = "dead"
                        self.infection_severity = "asymptomatic"

            dead_test = numpy.random.random()
            if economyModel.age_death_probs[int(indice)] > dead_test:
                self.infection_status = "dead"
                self.infection_severity = "asymptomatic"
                return
    ...
    def interact(self):
        ...
        cellmates = self.models.grid.get_neighbors(pos = [self.pos], moore = 1, radius = self.models.dist_spread, include_center = True)
        # print(len(cellmates))
        if len(cellmates) > 1:
...
chart2 = ChartModule([{'Label': 'economy',
                       'Color': 'Green'}],
  data_collector_name='datacollector')
...
model_parameters={
                ...
                "prob_spread": Slider("Probability Spread", 0.5, 0, 1, 0.05),
                "dist_spread": Slider("Spread Distance", 1, 0, 15, 1),
